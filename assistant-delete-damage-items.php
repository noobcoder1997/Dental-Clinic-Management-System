<?php
    ...
    try{
        ...
        $query = "SELECT * FROM damage_items WHERE item_id = ? ";
        $stmt0=$conn->prepare($query);
        $stmt0->bind_param('s',$id);
        $stmt0->execute();
        $result0=$stmt0->get_result();
        $row0=$result0->fetch_assoc();
        $name = $row0['product_name'];
        ...
        if($position == 'assistant'){
            ...
            if($row=$result->fetch_assoc()){
                $date = date('Y-m-d');
                $transaction = "Assistant ID #$_SESSION[id] (".strtoupper($row['first_name']).")- deleted a damaged item, product name: $name on $date at".date('H:i:s A');
            }
            $status = '1';
        ...
        $stmt = $conn->prepare("DELETE FROM damage_items WHERE item_id = ? ");
