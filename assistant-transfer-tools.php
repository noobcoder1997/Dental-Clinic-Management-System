<?php
    ...
    $query1 = mysqli_query($conn, "SELECT * FROM tools WHERE tool_id = '$id' LIMIT 1 ");
    while($row = mysqli_fetch_assoc($query1)){

        $stmt = $conn->prepare("INSERT INTO delivery (stock_no, description, quantity, product_name, branch_id, date) VALUES (?,?,?,?,?,?) ");
        $stmt->bind_param('ssssss', $id, $row['description'], $qty, $row['tool_name'], $branch, $date);
        $stmt->execute();

        echo "Tool successfully transferred!";

        $message = "Hi! A tool/s has been successfully transferred to your branch. Tool ID: $id, Tool Name: $row[tool_name] Quantity: $qty.";
        $showBtn=1;
        ...
        $message = "Hi! A tool/s has been successfully transferred to branch ID: $row1[location]. Tool ID: $id, Tool Name: $row[tool_name] Quantity: $qty.";

        $stmt1 = $conn->prepare("INSERT INTO notifications (patient_id, message) VALUES (?, ?) ");
        $stmt1->bind_param('ss',$designation, $message);
        $stmt1->execute();
        
        $transaction = "Assistant ID #$asst_id ($AsstName)- transfered a tool/s to $row1[location] on $date at $time";
    }
    ...
    $stmt2 = $conn->prepare("UPDATE tools SET quantity = quantity - ? WHERE tool_id = ? LIMIT 1");
    ...
    while ($row2 = $result2->fetch_assoc()) {
        if($row2['quantity'] < 1){
            ...
            $stmt3 = $conn->prepare("UPDATE tools SET quantity = quantity + ? WHERE tool_id = ? ORDER BY date ASC LIMIT 1");
